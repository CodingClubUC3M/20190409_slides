...
### Number of clicks by Hour by Net
...
renderPlotly({
  ...
  if (is.null(input$file1)){
    ...
    dd<-LogFile.F()
    nttdf<-data.frame(WD=dd$WeekDay,H=dd$Hour,N=dd$Net, Freq=1)
    nttdf %>% group_by(WD, H,N) %>%
    summarise(n = sum(Freq)) -> ntit3d
    # ntit3d
    ntit3d %>% group_by(N, H) %>%
    summarise(n =sum(n)) -> ntit2d
    # ntit2d
    za<-table(ntit2d$N,ntit2d$H)
    zaf<-row.names(za)
    zac<-colnames(za)
    
    for (k in c(1:dim(ntit2d)[1])){
      a<-c(1:2)[zaf==ntit2d$N[k]]
